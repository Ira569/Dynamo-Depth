...
def get_vis(opt, trainer, inputs, ref_frame_id, scale=0, items=('img', 'disp', 'ego_flow', 'ind_flow', 'mask')):
    ...
    if any(['flow' in it for it in items]):
        ...
        time_step = inputs[('ts', f_id)].reshape(-1, 1,
                                                 1).float()  # (B, 1, 1) time step from 0 to f_id - from dataloader, never close to 0
        camTcam = transformation_from_parameters(axisangle / time_step, translation / time_step,
                                                 invert=True)  # (B, 4, 4)

        collection['depth'] = depth
        # Ego-motion / Rigid Flow
        if 'ego_flow' in items:
            _, ego_flow_hsv, ego_flow_mag = trainer.vis_motion(depth=depth, K=K, inv_K=inv_K, motion_map=None,
                                                               camTcam=camTcam, scale=s)  # (1, 3, H, W), float
        ...
        if 'ind_flow' in items or 'samp_flow' in items:
            ...
            independ_flow = outputs[('motion_mask', f_id, s)] * (
                        outputs[('complete_flow', f_id, s)] - ego_flow.reshape(-1, 3, opt.height,
                                                                               opt.width))  # (B, 3, H, W)
            _, ind_flow_hsv, ind_flow_mag = trainer.vis_motion(depth=depth, K=K, inv_K=inv_K, motion_map=independ_flow,
                                                               camTcam=None, scale=s)  # (1, 3, H, W), float
            collection['ind_flow'] = {'hsv': ind_flow_hsv, 'mag': ind_flow_mag}

        # Complete Flow
        if 'comp_flow' in items:
            complete_flow = outputs[('complete_flow', f_id, s)]
            _, comp_flow_hsv, comp_flow_mag = trainer.vis_motion(depth=depth, K=K, inv_K=inv_K,
                                                                 motion_map=complete_flow, camTcam=None,
                                                                 scale=s)  # (1, 3, H, W), float
            collection['comp_flow'] = {'hsv': comp_flow_hsv, 'mag': comp_flow_mag}

        # Ego-motion / Rigid Flow + Independent Flow
        if 'samp_flow' in items:
            _, samp_flow_hsv, samp_flow_mag = trainer.vis_motion(depth=depth, K=K, inv_K=inv_K,
                                                                 motion_map=independ_flow, camTcam=camTcam,
                                                                 scale=s)  # (1, 3, H, W), float
...
def combine_vis(vis_list, arrangement, consistent_flow=True, flow_mag_factor=1.0, mask_max_mag=1.0):
    ...
    if consistent_flow and any(['flow' in a for arr in arrangement for a in arr]):
        max_flow_mag = max(
            [max([vis[a]['mag'] for arr in arrangement for a in arr if 'flow' in a]) for vis in vis_list])
            ...
            for a in arr:
                ...
                if 'img' in a:
                    ...
                    if consistent_flow:
                        ...
                        max_mag = flow_mag_factor * max(
                            [vis[a]['mag'] for arr in arrangement for a in arr if 'flow' in a])
...
# todo 获得第一个场景的第一张图的估计深度图，然后获得该帧的gt—label,然后将六张图的所有点放在一起计算IoU，或者用rhino软件可视化
